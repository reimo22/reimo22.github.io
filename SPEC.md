...
| `/writeups/` | Index of HTB writeups (box, OS, difficulty, technique), pulled at build time from `github.com/reimo22/htb-writeups` |
...
- `github.com/reimo22/htb-writeups` is mounted as a **git submodule** at
  ...
  with no glue code).
- `src/writeups/writeups.11tydata.js` cascades `layout`, `tags: "writeups"`, and
  a computed `/writeups/<box>/` permalink into every submodule subdirectory.
  The submodule's own files are never modified.
- Images pass through via `addPassthroughCopy` so the existing relative
  `./images/...` links in each `README.md` resolve unchanged.
- The writeups index is generated by **parsing the upstream root `README.md`
  table** (box / OS / difficulty / technique) — no front matter is added
  upstream, and writeups continue to be written exactly as today.
- The parser **fails the build** if the box-directory count and table-row count
  disagree, rather than silently dropping metadata.
