...
Eleventy discovers each `<box>/README.md` as a template with no glue code. The submodule
itself is never modified — instead `src/writeups/writeups.11tydata.js` sits one level *above*
it and cascades into every subdirectory, setting `layout: writeup.njk`, `tags: "writeups"`,
and a computed `permalink` of `/writeups/<box>/`.
...
The writeups index is generated by **parsing the upstream root `README.md` table**, which
already carries box / OS / difficulty / technique for all seven boxes. A small parser in
`.eleventy.js` turns it into a global data object keyed by directory name; the index page and
each writeup's header read from it. This means **zero changes to `htb-writeups`** — no front
matter added to files that currently render cleanly on GitHub, and you keep writing writeups
there exactly as you do now.

The parser **fails the build on a mismatch** between box directories and table rows, rather
than degrading quietly. If upstream reformats the table, adds a column, or adds a box without
a row, CI goes red with a message naming the offending box — instead of shipping seven
writeups stripped of their metadata behind a green check. A silent fallback here would
undercut the whole point of having gates that can fail.
...
5. **Writeups.** Add the submodule, the data cascade, image passthrough, README-table parser,
   generated index. No changes to the `htb-writeups` repo.
...
- `npx @11ty/eleventy --serve` — click every nav link; confirm each writeup renders with its
  images; confirm the index lists all 7 boxes.
